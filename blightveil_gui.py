...
# Function to parse log lines for contested zone elevators with dynamic manager ID
def parse_contested_zone_elevator(line):
    global last_log  # Use the global last_log

    # Check for "TransitManager_TransitDungeon" with dynamic manager IDs in the log
    if "TransitManager_TransitDungeon" in line:
        match = re.search(r"TransitManager_TransitDungeon([^\s]+)", line)
        if match:
            last_log['manager'] = match.group(1)  # Store the dynamic manager ID

    # Check for "TransitDungeon_Exfil_" (exiting the contested zone)
    elif "TransitDungeon_Exfil_" in line:
        highlight_log(f"🚪 **Exit Notice**: Someone has exited the Contested Zone (CZ)", 'orange')

    # Check for "TransitDungeonRewardRoom_" (in a loot room)
    elif "TransitDungeonRewardRoom_" in line:
        highlight_log(f"💎 **Loot Room Alert**: Someone is in a Loot Room", 'yellow')

    # Check for "Opened:" or "Closed:" with no elevator info
    elif "Opened:" in line or "Closed:" in line:
        if last_log.get('manager'):
            state = "Opened" if "Opened:" in line else "Closed"
            highlight_log(f"🚪 **CZ Elevator Moving**: {state} for manager {last_log['manager']}", 'orange')
            last_log['manager'] = None  # Reset after handling
...
def parse_kill_line(line):
    ...
    # First, check for contested zone elevator lines
